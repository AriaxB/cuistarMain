...
if (isset($_GET['busqueda'])) {
    ...
    $busqueda = mysqli_real_escape_string($conexion, $_GET['busqueda']);
            ...
            $sql = "SELECT si.id_stock, si.id_producto, p.nombre AS nombre_producto, p.descripcion AS descripcion_producto, p.precio, p.imagen, si.cantidad, si.salidas, si.Fecha_de_entrada, si.Fecha_de_salida, si.valor_total, si.Fecha_de_vencimiento
            FROM stock_inventario si
            LEFT JOIN productos p ON si.id_producto = p.id_producto
            ...
            <div class="tablas-informativas">
                ...
                <form action="" method="GET" onsubmit="buscarProducto()">
                    <div style="display: flex;justify-content: flex-start;align-items: center;margin-right: 27%;">
                        ...
                        <div>   
                            <a href="agregarInventario.php" class="boton-login">Agregar</a>
                        </div>
                    </div>
                </form>
                <table id="t-all">
                    <thead>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>{$row['Fecha_de_salida']}</td>";
                            echo "<td>{$row['Fecha_de_vencimiento']}</td>";
                            echo "<td>";
                            echo "<button onclick=\"window.location.href='actualizarInventario.php?id_stock={$row['id_stock']}'\" class='boton-login'>Actualizar</button>";
                            echo "<form action='../php/eliminarInventario.php' method='POST'>";
                            echo "<input type='hidden' name='id_stock' value='{$row['id_stock']}'>";
                            echo "<button type='submit' onclick=\"return confirm('¿Estás seguro de que quieres eliminar este registro?');\" class='boton-login'>Eliminar</button>";
                            echo "</form>";                            
                            echo "</td>";
                            echo "</tr>";
                        }
